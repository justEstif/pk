...
type Harness = 'claude' | 'codex' | 'cursor' | 'opencode' | 'pi'
...
const HARNESS_DESCRIPTIONS: Record<Harness, string> = {
  ...
  pi:     'Pi / Oh My Pi — before_agent_start extension',
}
...
function installPiExtension(): void {
  // Project-local: .pi/extensions/ (Pi) or .omp/extensions/ (Oh My Pi)
  const dir = path.join('.pi', 'extensions')
  mkdirSync(dir, { recursive: true })
  const file = path.join(dir, 'pk.ts')
  writeFileSync(file, PI_EXTENSION)
  console.log(`wrote ${file}`)
  console.log(`Oh My Pi users: also copy to .omp/extensions/pk.ts`)
}

...
export function checkTags(tags: unknown, p: string): Issue[] {
   if (!Array.isArray(tags)) return [issue('error', 'tags must be a flat list', p)]
   if (tags.length === 0) return [issue('warn', 'tags is empty', p)]
   return []
}
...
export function checkSourceExtracted(body: string, meta: Record<string, unknown>, p: string): Issue[] {
   if (meta.type !== 'source' || meta.status !== 'processed') return []
   const m = body.match(/^## Extracted Items\s*\n([\s\S]*?)(?=^##\s|$)/m)
   if (m && !m[1]?.trim()) return [issue('warn', 'processed source has empty Extracted Items', p)]
   return []
}
...
export function lintNotes(knowledgeDir: string): { issues: Issue[]; noteCount: number } {
   ...
   for (const n of notes) {
      const p = n.path
      if (n.err) { issues.push(issue('error', `parse error: ${n.err}`, p)); continue }

      issues.push(...checkRequiredFields(n.meta, p))

      const type = (n.meta.type ?? '') as string
      if (!TYPE_DIRS[type]) { issues.push(issue('error', `invalid type: ${type}`, p)); continue }

      issues.push(...checkTypeAndLocation(n.meta, p, knowledgeDir))

      const id = (n.meta.id ?? '') as string
      if (id) {
         if (ids.has(id)) issues.push(issue('error', `duplicate id ${id} (also in ${ids.get(id)})`, p))
         else ids.set(id, p)
      }

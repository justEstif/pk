  ...
  program
    ...
    .action((query, opts) => {
      ...
      if (opts.sessionStart) {
        ...
        notes = all.filter(n =>
          n.meta.status === 'open' || n.meta.status === 'accepted' || n.meta.status === 'active'
        ...
        console.error('Provide a query or --all'); process.exit(1)
      }
      if (!notes || notes.length === 0) { console.log('No matching notes.'); return }

...
const TYPE_GUIDE = 'Types: note (Summary/Details/Evidence/Related), decision (Decision/Context/Rationale/Consequences/Related), question (Question/Why It Matters/Current Understanding/Resolution), source (Source/Raw Material/Extracted Items). Tags: lowercase slugs e.g. "auth,api". Run pk_lint after pk_write.';
...
function createPkMcpServer(): McpServer {
	...
	server.tool(
		...
		async ({query, type, status, tag, limit}) =>
			runMcpTool('pk_search', {
				query, type, status, tag, limit,
			}, async dir => {
	...
	server.tool(
		...
		async ({query, sessionStart, all, type, tag, limit}) =>
			runMcpTool('pk_synthesize', {
				query, sessionStart, all, limit,
			}, async dir => {
				const notes = await selectNotes(dir, query, {
					all, limit, sessionStart, tag, type,
				});
				if (notes.length === 0) {

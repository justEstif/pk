...
/**
 * After any mutation, regenerate ${CLAUDE_PROJECT_DIR}/.claude/rules/pk.md
 * so Cowork's native InstructionsLoaded mechanism picks up fresh context
 * at the next session start — no SessionStart hook needed.
 *
 * No-op when CLAUDE_PROJECT_DIR is not set (terminal harness users).
 */
async function syncCoworkRules(dir: string): Promise<void> {
...
pk is set up for this project. The knowledge base is currently empty.

As the user works, offer to save important context: decisions, reasons behind
choices, open questions, meeting outcomes, and useful references.

When the user mentions a decision or conclusion, say something like:
"Want me to save that to your project knowledge? That way future sessions will
have context on why you chose this."

Tools available:
- pk_new — create a note (types: note, decision, question, source)
- pk_search — search existing knowledge
- pk_read — read a full note
- pk_write — update a note
- pk_lint — validate note structure`
			: formatSynthesizeOutput(notes, 'session context');
		const header = '<!-- Auto-generated by pk — edit notes via pk MCP tools, not this file -->\n\n';
		await Bun.write(path.join(rulesDir, 'pk.md'), header + body);
	} catch {
		// Non-fatal — session priming degrades gracefully if synthesis fails
	}
}

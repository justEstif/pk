...
import type { Note, NoteMeta } from './notes.ts'
import { parseFrontmatter, slugify, excerpt, TYPE_DIRS } from './notes.ts'
...
describe('excerpt', () => {
   const makeNote = (type: string, body: string): Note => ({
      body,
      meta: { type, id: 'x', title: 'T', status: 'active', tags: [], created: '2024-01-01', updated: '2024-01-01' },
      path: '/tmp/x.md',
   })
...
describe('TYPE_DIRS', () => {
   test('contains all five canonical types', () => {
      expect(Object.keys(TYPE_DIRS).sort()).toEqual(['decision', 'index', 'note', 'question', 'source'])
   })
})

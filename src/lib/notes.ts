...
export function parseFrontmatter(text: string): { body: string; meta: NoteMeta } | { err: string } {
  ...
  if (end === -1) return { err: 'missing closing frontmatter delimiter' }
  ...
  for (const line of raw.split('\n')) {
    ...
    if (colon === -1) return { err: `invalid frontmatter line: ${JSON.stringify(trimmed)}` }
...
export function allNotes(knowledgeDir: string): Note[] {
  return walkMd(knowledgeDir).map((p) => {
    ...
    if ('err' in result) return { body: '', err: result.err, meta: {}, path: p }
    return { ...result, path: p }
  })
}

export function validNotes(knowledgeDir: string, excludeTypes: string[] = []): Note[] {
  return allNotes(knowledgeDir).filter(
    (n) => !n.err && !excludeTypes.includes(n.meta.type ?? ''),
  )
}
